...
app
  ...
  .post(checkAuth, async function (req, res) {
    ...
    if (req.body.MiiType == "CMOC") {
      ...
      await renderGen2Mii(req.body.miidata, req.user.id, dataFolder).catch(
        (err) => {
...
app.get("/create", checkAuth, async function (req, res) {
  ...
  if (!exists) {
    await coinDb.insert(
      "coins",
      ["snowflake", "count"],
      [req.user.id, getUserData(req.user.id).coins]
    );
  }
  getTag(req.user.id, res).catch((err) => {
    if (err == "Redirect") {
...
app.listen(port, async function () {
  ...
  await gameDb.create("games", [
    "id INTEGER PRIMARY KEY",
    "console INTEGER",
    "gameID TEXT",
    "count INTEGER",
  ]);
  await coinDb.create("coins", [
    "id INTEGER PRIMARY KEY",
    "snowflake TEXT",
    "count INTEGER",
  ]);
...
function getOverlayList() {
  var overlays = [];
  fs.readdirSync(path.resolve(dataFolder, "overlays")).forEach(function (
    overlayFile
  ) {
    overlays.push(
      JSON.parse(
...
app.use(function (req, res, next) {
  var allowed = ["/img", "/overlays", "/flags"];
