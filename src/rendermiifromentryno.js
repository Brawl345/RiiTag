...
module.exports = async function (entrynumber, id, dataFolder) {
  ...
  const isValid = regex.test(entrynumber);
  if (!isValid) {
  ...
  const requestSettings = {
    ...
    headers: {
      'Content-Type':
        'multipart/form-data; boundary=----WebKitFormBoundary2bpPmOcP6IdUsMAq',
      'Accept': '*/*',
    },
    body:
  ...
  request.post(requestSettings, async (error, response, body) => {
